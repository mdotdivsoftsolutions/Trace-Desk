...
import { Layers, Plus, Calendar, CheckCircle2, Receipt, Edit, Trash2 } from 'lucide-react';
        ...
        {milestones.map((m, idx) => (
          <div key={m._id} className="p-4 rounded-lg bg-white dark:bg-[#1E293B] border border-neutral-200 dark:border-[#334155] shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="space-y-1">
              ...
              {m.description && <p className="text-xs text-neutral-500 dark:text-neutral-400">{m.description}</p>}
              {m.dueDate && (
            ...
            <div className="flex items-center gap-3 self-start md:self-auto flex-shrink-0">
              <div className="text-right">
                ...
                <span className="font-extrabold text-neutral-900 dark:text-white font-mono">{formatCurrency(m.amount)}</span>
              </div>
              <div className="flex items-center gap-1">
                {m.status === 'completed' && !m.invoiceId && (
                ...
                <button onClick={() => onEditMilestone(m)} className="p-1.5 rounded text-neutral-400 hover:text-neutral-900 dark:hover:text-white"><Edit className="w-4 h-4" /></button>
                <button onClick={() => onDeleteMilestone(m)} className="p-1.5 rounded text-neutral-400 hover:text-rose-600"><Trash2 className="w-4 h-4" /></button>
              </div>
            </div>
          </div>
        ))}
